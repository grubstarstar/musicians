...
/**
 * Applied-tab copy of the request detail screen (MUS-64). Functionally the
 * same as `(tabs)/(stack)/request/[id].tsx` but mounted inside the Applied
 * stack so back-nav returns to the Applied list instead of bouncing to the
 * Home/Notices stack. When the user has already applied, the server returns
 * CONFLICT on resubmit and the UI collapses to "Interest sent" — i.e. the
 * "already-applied mode" the ticket describes falls out of the existing code.
 */
export default function AppliedDetailScreen() {
    ...
    trpc.expressionsOfInterest.create.mutationOptions({
      onSuccess: () => {
        ...
        queryClient.invalidateQueries({
          queryKey: requestQueryOptions.queryKey,
        });
        queryClient.invalidateQueries({
          queryKey: trpc.expressionsOfInterest.listMine.queryOptions().queryKey,
        });
      },
      onError: (err) => {
        const message = err.message;
        // CONFLICT means the caller already has a pending EoI on this request —
        // treat as submitted so the UI collapses to the "Interest sent" state.
